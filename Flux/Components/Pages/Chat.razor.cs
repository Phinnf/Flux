...
{
    [Inject] private WorkspaceClientService WorkspaceService { get; set; } = default!;
    ...
    private List<MessageDto> _rootMessages = new();
    private List<MessageDto> RootMessages => _rootMessages;

    private void UpdateRootMessages()
    {
        _rootMessages = _messages
    ...
    private MessageMeta GetMessageMeta(MessageDto msg)
    {
        var index = _rootMessages.IndexOf(msg);
        var prevMsg = index > 0 ? _rootMessages[index - 1] : null;
        ...
        if (showDate)
        {
            dateLabel = msg.CreatedAt.Date == DateTime.Today ? "Today" : 
                        msg.CreatedAt.Date == DateTime.Today.AddDays(-1) ? "Yesterday" : 
                        msg.CreatedAt.ToString("dddd, MMMM d");
        }

        return new MessageMeta { ShowDate = showDate, DateLabel = dateLabel, IsCompact = isCompact };
    }

    private async Task HandleQuickActionById(Guid messageId)
    {
        var msg = _messages.FirstOrDefault(m => m.Id == messageId);
        if (msg != null)
        {
            await HandleQuickAction(msg, "Delete");
        }
    }
    ...
    private async Task OnChatInputChanged(string value)
    {
    ...
    private void CheckForMentions()
    {
        ...
        if (lastAtSymbol >= 0)
        {
            bool isStartOfWord = lastAtSymbol == 0 || char.IsWhiteSpace(_newMessageContent[lastAtSymbol - 1]);
            if (isStartOfWord)
            {
                ...
                if (!_mentionSearchQuery.Contains(" ") && !_mentionSearchQuery.Contains("\n"))
                {
                    ...
                    if (_filteredMembers.Any())
                    {
                        ...
                        if (!_filteredMembers.Any(m => m.Id == _selectedMentionId))
                        {
    ...
    private async Task InsertMention(string username)
    {
        var lastAtSymbol = _newMessageContent.LastIndexOf('@');
        if (lastAtSymbol >= 0)
        {
    ...
    {
        if (!StateService.Members.Any())
        {
            _ = StateService.RefreshMembersAsync(WorkspaceId);
        }
        _showMentionsList = !_showMentionsList;
        if (_showMentionsList)
        {
            if (!string.IsNullOrEmpty(_newMessageContent) && !_newMessageContent.EndsWith("@") && !_newMessageContent.EndsWith(" "))
            {
                _newMessageContent += " ";
            }
        ...
        else if (_newMessageContent.EndsWith("@"))
        {
    ...
    private async Task ApplyFormat(string prefix, string suffix = "")
    {
        ...
        var newContent = await JS.InvokeAsync<string>("chatHelper.insertTextAtCursor", _textAreaRef, prefix, suffix);
        if (newContent != null)
        {
            _newMessageContent = newContent;
            StateHasChanged();
        }
    }
    ...
    private async Task SwitchCaptureMode(string mode)
    {
        ...
        _isCameraError = false;
        StateHasChanged();
        bool started = mode == "camera" 
            ? await JS.InvokeAsync<bool>("window.videoRecorder.startCamera", "videoPreview")
            : await JS.InvokeAsync<bool>("window.videoRecorder.startScreenShare", "videoPreview");
    ...
    private async Task StartRecording()
    {
        ...
        if (await JS.InvokeAsync<bool>("window.videoRecorder.startRecording"))
        {
            _isRecording = true;
            _recordingSeconds = 0;
            _recordingDurationString = "00:00";
    ...
    {
        if (_recordedVideoUrl != null) await JS.InvokeVoidAsync("window.videoRecorder.downloadVideo", _recordedVideoUrl);
    }
    ...
    private async Task UploadVideo()
    {
        ...
        else ToastService.ShowError("Failed to upload video.");
        StateHasChanged();
    }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        if (user.Identity is { IsAuthenticated: true })
        {
            ...
            if (idClaim != null && Guid.TryParse(idClaim.Value, out var id)) CurrentUserId = id;
        }
        ...
        _hubConnection.On<SendMessageResponse>("ReceiveMessage", async (response) =>
        {
            if (response.ChannelId == ChannelId && !_messages.Any(m => m.Id == response.Id))
            {
                ...
                if (response.ParentMessageId != null)
                {
                    ...
                    if (parentIndex != -1) _messages[parentIndex] = _messages[parentIndex] with { ReplyCount = _messages[parentIndex].ReplyCount + 1 };
                }
                UpdateRootMessages();
        ...
        _hubConnection.On<ToggleReactionResponseDto>("ReactionToggled", async (response) =>
        {
            ...
            if (index != -1)
            {
                ...
                if (response.IsAdded) { if (!newReactions.Any(r => r.Emoji == response.Emoji && r.UserId == response.UserId)) newReactions.Add(new ReactionDto(response.UserId, response.Emoji)); }
                else newReactions.RemoveAll(r => r.Emoji == response.Emoji && r.UserId == response.UserId);
                _messages[index] = msg with { Reactions = newReactions };
                UpdateRootMessages();
                await InvokeAsync(async () => { StateHasChanged(); if (_messageVirtualizeRef != null) await _messageVirtualizeRef.RefreshDataAsync(); });
            }
        });
    ...
    protected override async Task OnParametersSetAsync()
    {
        ...
        {
            if (_currentConnectedChannelId != Guid.Empty && _currentConnectedChannelId != ChannelId) await _hubConnection.InvokeAsync("LeaveChannel", _currentConnectedChannelId.ToString());
            if (_currentConnectedChannelId != ChannelId) { await _hubConnection.InvokeAsync("JoinChannel", ChannelId.ToString()); _currentConnectedChannelId = ChannelId; }
        }
        else _currentConnectedChannelId = ChannelId;
        await LoadChatData();
    }
    ...
    private async Task LoadMoreMessages()
    {
        ...
        try
        {
            var result = await MessageService.GetMessagesAsync(ChannelId, _messages.First().CreatedAt);
            if (result.IsSuccess && result.Value != null && result.Value.Any())
            {
                _messages.InsertRange(0, result.Value);
                _hasMoreMessages = result.Value.Count >= 50;
                UpdateRootMessages();
            }
            else _hasMoreMessages = false;
        }
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            ...
            await JS.InvokeVoidAsync("window.registerEnterKeyHandler", _textAreaRef, _objRef);
        }
        if (_shouldScrollToBottom) { _shouldScrollToBottom = false; await ScrollToBottom(); }
    }
    ...
    private async Task SendMessage()
    {
        ...
        await JS.InvokeVoidAsync("chatHelper.resetTextAreaHeight", _textAreaRef);
        await InvokeAsync(StateHasChanged);
        var result = await MessageService.SendMessageAsync(new SendMessageCommand(content, ChannelId, CurrentUserId));
        if (!result.IsSuccess)
        {
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        {
            try { if (_hubConnection.State == HubConnectionState.Connected) await _hubConnection.InvokeAsync("LeaveChannel", ChannelId.ToString()); } catch { }
    ...
    private void OpenMessageMenu(MouseEventArgs e, MessageDto message) { _selectedMessage = message; _menuX = e.ClientX; _menuY = e.ClientY; _showMessageMenu = true; }
    private void CloseMessageMenu() { _showMessageMenu = false; _selectedMessage = null; }

    private async Task HandleQuickAction(MessageDto msg, string action) { _selectedMessage = msg; await HandleAction(action); }
    ...
    private async Task HandleAction(string action)
    {
        ...
        switch (action)
        {
            case "Reply": await OpenThread(msg); break;
            case "Edit": _editingMessageId = msg.Id; _editMessageContent = msg.Content; break;
            case "Delete": await ConfirmDeleteMessage(msg.Id); break;
        }
    }

    private void CancelEdit() { _editingMessageId = Guid.Empty; _editMessageContent = ""; StateHasChanged(); }

    private async Task SaveEditMessage()
    {
        if (string.IsNullOrWhiteSpace(_editMessageContent)) return;
        var result = await MessageService.EditMessageAsync(_editingMessageId, new Flux.Features.Messages.EditMessage.EditMessageRequest(_editMessageContent, CurrentUserId));
        if (result.IsSuccess)
        {
            var index = _messages.FindIndex(m => m.Id == _editingMessageId);
            if (index != -1) { _messages[index] = _messages[index] with { Content = _editMessageContent, UpdatedAt = DateTime.UtcNow }; UpdateRootMessages(); }
            ToastService.ShowSuccess("Message updated.");
        }
        else ToastService.ShowError($"Failed to edit message: {result.Error}");
    ...
    private async Task ConfirmDeleteMessage(Guid messageId) { _messageToDeleteId = messageId; _showDeleteDialog = true; await InvokeAsync(StateHasChanged); }
    ...
    private async Task ExecuteDeleteMessage()
    {
        ...
        if (result.IsSuccess) { if (_messages.RemoveAll(m => m.Id == _messageToDeleteId) > 0) UpdateRootMessages(); ToastService.ShowSuccess("Message deleted."); }
        else ToastService.ShowError($"Failed to delete message: {result.Error}");
        _showDeleteDialog = false;
        await InvokeAsync(StateHasChanged);
    }

    private bool _showEmojiPicker = false;
    private double _emojiPickerX = 0, _emojiPickerY = 0;
    ...
    private void CloseEmojiPicker() => _showEmojiPicker = false;
    ...
    public async Task OnEmojiSelectedJS(string emoji)
    {
        ...
        CloseEmojiPicker();
        if (targetId != default) await ToggleReaction(targetId, emoji);
        else { _newMessageContent += emoji; await InvokeAsync(StateHasChanged); }
    }
    ...
    private async Task ToggleReaction(Guid messageId, string emoji)
    {
        ...
        if (index != -1)
        {
            ...
            UpdateRootMessages();
            StateHasChanged();
            if (_messageVirtualizeRef != null) await _messageVirtualizeRef.RefreshDataAsync();
        }
        var result = await MessageService.ToggleReactionAsync(messageId, CurrentUserId, emoji);
        if (!result.IsSuccess) ToastService.ShowError("Failed to toggle reaction.");
    }
    ...
    {
        var result = await WorkspaceService.GetOrCreateDirectChannelAsync(WorkspaceId, CurrentUserId, memberId);
        if (result.IsSuccess) Navigation.NavigateTo($"/workspace/{WorkspaceId}/dms/{result.Value.Id}");
        else ToastService.ShowError("Failed to start DM.");
    }
}
